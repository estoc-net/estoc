/**
 * The procedures: what a device appends when a
 ...
 * they land. Plus the vault's `ImportPolicy`:
...
/** The roots the merged vault set holds. */
export function heldRoots(events: Event[]): Cid[] {
  return foldOf(events).held();
}

/** Every root an event of the set references: an extension's, whose erase the vault does not know. */
...
/**
 * What `importVault` asks and the vault's types answer: held roots by
 * the erases (vault's set) or whole (an extension's), the purged
 * extensions, the installed ones.
 */
...
// ---- observations that check the fold first ---------------------------------

/** `channel.firstSeen` once per device: appended unless `self` has one on this pair already. */
...
/** `peer.resolved`, written only when it differs from the pair's latest for that DID. */
...
/**
 * A message, body first: the plaintext into the blob store, then the
...
/**
 * Erase roots of one message: the event, appended; collection is a
...
/** `collect` with the fold's keep-set: unlink what no held root reaches, grace allowing. */
...
/** How a reader shows a root a line names: the erase asked first, the blocks second. */
...
export async function readRoot(blobs: BlobStore, fold: VaultFold, mid: string, root: Cid): Promise<Absence> {
  ...
  while (pending.length > 0) {
    ...
    try {
      ...
      return { state: "missing" }; // damage is absence to this rule
    }
  }
  ...
  for (const gone of fold.deletedContacts()) {
    for (const pair of gone.channels) {
      ...
      if (attribution.kind !== "deleted" || attribution.cids.length !== 1) {
        continue; // in conflict: left until the conflict is resolved
      }
...
/** The retirement step of deleting a contact, idempotent: retire every key of `keyList` that was the contact's alone and that no one else uses. */
async function retireDeleted(events: EventStore, fold: VaultFold, rep: string, keyList: { key: string; because: string }[]): Promise<Event[]> {
  ...
  for (const entry of keyList) {
    ...
    if (!oneUse && entry.because !== "minted") {
      continue; // not this contact's alone by minting or invitation
    }
...
/**
 * Delete a contact: a tombstone per member, the erases over its
 ...
 * and never widens to a member merged in after the tombstone.
 */
export async function deleteContact(vault: VaultSide, fold: VaultFold, cid: string): Promise<Deleted> {
  ...
  if (gone !== undefined) {
    ...
    // it names, so the live representative is not this deletion's to widen into.
  ...
  // a late merge can hang tombstoned members on a live contact: their keys are still this deletion's to retire
  ...
  // one write (`appendAll`): a crash leaves every member tombstoned or none — the tombstones cannot half-land
...
/**
 * Held after merge: one `delivery.held { imported }` from `self`
