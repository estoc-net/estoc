 ...
 * from the outbound fold; an event of a type this version does not
 * name, or one whose payload does not read, holds every root it names,
 * and so does a package in dispute, since contested evidence releases
 * nothing. Reading a root asks the same question from the other side:
 * erased before absent, so that missing bytes never read as a
 * deletion.
 */
...
export function heldRoots(set: VaultEventSet, outbound: OutboundFold, erasures: Erasures = foldErasures(set)): Set<Cid> {
  ...
  for (const type of ["message.out", "message.in"] as const) {
    for (const event of set.of(type)) for (const root of event.roots) if (!erased(erasures, event.data.messageId, root)) held.add(root);
  }
