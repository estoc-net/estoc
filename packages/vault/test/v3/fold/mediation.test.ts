...
import { Keys, foldMediations, mintMediationDid, verifyMediationKeys, type Did, type KeyName, type MediationId } from "../../../src/v3/index.js";
import { MEDIATION, MEDIATION2, ROUTING_DID, ROUTING_DID2, Scene, expectOrderFree, openKeys } from "./helpers.js";
...
describe("the mediation fold", () => {
  it("makes an arrangement usable with one consistent creation and one grant, pending before either", () => {
    const scene = new Scene();
    created(scene, MEDIATION, me);
    expect(foldMediations(scene.set()).mediations.get(MEDIATION)).toMatchObject({ status: "pending", routingDid: null, mediatorDid: MEDIATOR });
    scene.add("mediation.granted", { mediationId: MEDIATION, routingDid: ROUTING_DID });
    created(scene, MEDIATION, me);
    scene.add("mediation.granted", { mediationId: MEDIATION, routingDid: ROUTING_DID });
    const fold = foldMediations(scene.set());
    expect(fold.mediations.get(MEDIATION)).toMatchObject({ status: "usable", routingDid: ROUTING_DID, me: { did: me }, retired: null, faults: [], identity: "unchecked" });
    expect(fold.usable(MEDIATION)).toBe(true);
    expect(fold.usable(MEDIATION2)).toBe(false);
    expectOrderFree(scene.events, foldMediations);
  });
  ...
  it("makes disagreeing creations or grants a conflict that nothing later resolves", () => {
    const scene = new Scene();
    created(scene, MEDIATION, me);
  ...
  it("retires terminally and prefers the latest selection only while it is usable", () => {
    const scene = new Scene();
    created(scene, MEDIATION, me);
    scene.add("mediation.granted", { mediationId: MEDIATION, routingDid: ROUTING_DID });
    created(scene, MEDIATION2, me2);
    ...
    expect(foldMediations(scene.set()).preferred).toBe(MEDIATION);
  ...
  it("checks each arrangement's own DID against the seed and treats a mismatch as a conflict", async () => {
    const scene = new Scene();
    created(scene, MEDIATION, me);
    scene.add("mediation.granted", { mediationId: MEDIATION, routingDid: ROUTING_DID });
    created(scene, MEDIATION2, (await mintMediationDid(await openKeys(new Uint8Array(32).fill(8)), MEDIATION2)).longFormDid);
    scene.add("mediation.granted", { mediationId: MEDIATION2, routingDid: ROUTING_DID2 });
    const unchecked = foldMediations(scene.set());
    expect(unchecked.mediations.get(MEDIATION2)?.status).toBe("usable");
