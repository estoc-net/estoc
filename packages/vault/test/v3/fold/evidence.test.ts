...
describe("verifyResolutions", () => {
  it("verifies a long-form snapshot from the spelling alone, a short-form and a did:web snapshot only from the retained document", async () => {
    ...
    const readObject = readerOf(new Map([[web.resolution.cid, bytes], [b0.resolution.cid, b0.resolution.bytes]]));
    ...
    expect(await resolvedDocumentOf(fetched.data, readObject)).toEqual(document);
