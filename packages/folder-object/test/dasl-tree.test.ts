...
import { readObject, encodeCar, decodeCar } from "../src/index.js";
import {
  decodeManifest,
  encodeDrisl,
  encodeManifest,
...
function objectSet(files: TreeFiles, hashed: HashedManifest): Map<string, Uint8Array> {
...
describe("the manifest block", () => {
  ...
  it("encodeManifest takes entries in any order and checks them", async () => {
    ...
    const drisl = await (await import("../src/dasl/index.js")).drislCid(utf8("x"));
  ...
  it("refuses every shape that is not exactly a manifest", async () => {
    ...
    const drislLink = new Link(parseCid(await (await import("../src/dasl/index.js")).drislCid(utf8("x"))));
...
describe("verifyTree", () => {
  ...
  it("rejects a leaf whose length is not the size the manifest states", async () => {
    ...
    const root = await (await import("../src/dasl/index.js")).drislCid(manifest);
