...
**The DASL encoding** (`@estoc/folder-object/dasl`, experimental, spec branch `dasl`) is the same three layers with the tree hashed as [DASL](https://dasl.ing/) instead of UnixFS: every file one raw block hashed whole, the tree one DRISL document — a MASL bundle `{resources: {"/path": {src, size}}}` — and the root its drisl CID. No directory nodes, no chunking, no dependency; `hashTree`, `verifyTree`, `resolvePath`, `hashObject`, `signObject`, `verifyObjectCard` have the same names and the manifest in place of the UnixFS DAG. The two encodings are told apart by the root's codec (`bafybei…` dag-pb is UnixFS, `bafyrei…` drisl is a manifest).
