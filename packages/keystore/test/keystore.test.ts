...
// Fast KDF for tests; production default is 210k iterations.
const FAST = { pbes2Iterations: 2048 };
...
describe("keystore", () => {
  ...
  it("lists names and DIDs without a passphrase", async () => {
    ...
    ({ doc } = await createKey(doc, "root", "pw", FAST));
    ({ doc } = await createKey(doc, "work", "pw", FAST));
  ...
  it("rejects a wrong passphrase without leaking jose internals", async () => {
    const { doc } = await createKey(emptyKeystore(), "root", "right", FAST);
  ...
  it("rejects duplicate names, unknown names, and empty names", async () => {
    const { doc } = await createKey(emptyKeystore(), "root", "pw", FAST);
    await expect(createKey(doc, "root", "pw", FAST)).rejects.toThrow(/already exists/);
    await expect(openKey(doc, "nope", "pw")).rejects.toThrow(/no key named/);
    await expect(createKey(doc, "", "pw", FAST)).rejects.toThrow(/must not be empty/);
    expect(() => removeKey(doc, "nope")).toThrow(/no key named/);
  });

  it("removeKey drops the entry and leaves the input untouched", async () => {
    let doc = emptyKeystore();
    ({ doc } = await createKey(doc, "a", "pw", FAST));
    ({ doc } = await createKey(doc, "b", "pw", FAST));
  ...
  it("two keys agree on the same X25519 shared secret", async () => {
    const { signer: alice } = await createKey(emptyKeystore(), "a", "pw", FAST);
    const { signer: bob } = await createKey(emptyKeystore(), "b", "pw", FAST);
  ...
  it("accepts a caller-supplied private key and stamps a fixed time", async () => {
    ...
    const again = await createKey(emptyKeystore(), "fixed", "pw", { ...FAST, privateKey });
