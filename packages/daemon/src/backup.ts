...
/**
 * The backup file: the vault's snapshot zipped, nothing converted. Its
 * entries are the vault's paths as they are (`.estoc/config.json`, …)
 * — everything but `local/` — so unzipping it on a laptop yields the
 * folder every Estoc client reads. Reading one
