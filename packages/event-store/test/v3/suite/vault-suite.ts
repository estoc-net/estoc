 ...
 * collection deletes exactly the unkept while a paused reader blocks
 * nothing, mutations through a held view run in the order issued, the
 * keystore rewraps under the lock, and ingest reads its input as it
 * was yielded. The opener supplies a runtime with its author and clock
 * pinned, and a way to damage an accepted object's stored bytes as a
 * bad sector would.
 */
...
export interface VaultUnderTest {
  vault: Runtime;
  /** Damage an accepted object's stored bytes in place, as a bad sector would, without telling the store. */
...
export async function rootsOf(held: Held): Promise<Cid[]> {
  const roots: Cid[] = [];
