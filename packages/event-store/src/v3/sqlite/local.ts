...
/** One entry of the trace: its place in the trace, when it was written by the runtime's clock, what it is and what it says. */
...
export interface LocalState {
  ...
  /** Empties the cache and the trace. The options, the identity, the control and the keystore stay. */
...
export class SqliteLocalState implements LocalState {
  ...
  constructor(
    ...
    this.options = {
      get: async (key) => {
        const row = this.reading(OPTIONS) ? query(this.driver, `SELECT value FROM ${OPTIONS} WHERE key = ?`, key)[0] : undefined;
        return row === undefined ? undefined : (parseStrict(row["value"] as Uint8Array) as JsonValue);
      },
      set: async (key, value) => {
        const bytes = canonicalize(value); // refused as JSON before anything is written
        this.writing(OPTIONS, "options.set");
        run(this.driver, `INSERT OR REPLACE INTO ${OPTIONS} (key, value) VALUES (?, ?)`, key, bytes);
      },
      delete: async (key) => {
        this.writing(OPTIONS, "options.delete");
        run(this.driver, `DELETE FROM ${OPTIONS} WHERE key = ?`, key);
      },
      keys: async () => (this.reading(OPTIONS) ? query(this.driver, `SELECT key FROM ${OPTIONS} ORDER BY key`).map((row) => row["key"] as string) : []),
    };
    this.cache = {
      get: async (namespace, key) => {
        const row = this.reading(CACHE) ? query(this.driver, `SELECT bytes FROM ${CACHE} WHERE namespace = ? AND key = ?`, namespace, key)[0] : undefined;
        return row === undefined ? undefined : (row["bytes"] as Uint8Array);
      },
      put: async (namespace, key, bytes) => {
        this.writing(CACHE, "cache.put");
        run(this.driver, `INSERT OR REPLACE INTO ${CACHE} (namespace, key, bytes) VALUES (?, ?, ?)`, namespace, key, bytes);
      },
      delete: async (namespace, key) => {
        this.writing(CACHE, "cache.delete");
        run(this.driver, `DELETE FROM ${CACHE} WHERE namespace = ? AND key = ?`, namespace, key);
      },
      clear: async (namespace) => {
        this.writing(CACHE, "cache.clear");
    ...
    this.trace = {
      append: async (type, data) => {
        ...
        this.writing(TRACE, "trace.append");
        const at = atOf(this.now());
        const [row] = query(this.driver, `INSERT INTO ${TRACE} (seq, at, type, data) SELECT coalesce(max(seq), 0) + 1, ?, ?, ? FROM ${TRACE} RETURNING seq`, at, type, bytes);
        return { seq: row?.["seq"] as number, at, type, data: parseStrict(bytes) as JsonObject };
      },
      ...
      prune: async (policy) => {
        ...
        this.writing(TRACE, "trace.prune");
  ...
  /** Whether a read of `table` has anything to read: the vault is open, and the table has been made. */
  private reading(table: string): boolean {
    this.check();
    return this.made.has(table) || hasTable(this.driver, table);
  }

  /** Before a write to `table`: the vault is open and writable, and the table exists. */
  private writing(table: string, what: string): void {
    this.check();
