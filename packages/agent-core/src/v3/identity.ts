/**
 * The version-3 vault as the agent opens it: the SQLite runtime under
 * `@estoc/event-store/v3` with the seed's keys under `@estoc/vault/v3`.
...
import {
  ...
  type Validated,
  type WrappedSeed,
} from "@estoc/event-store/v3";
import { unlockSeedKeystore, type SeedKey } from "@estoc/keystore";
import { Keys, scanVault, vaultDraft, vaultHeldRoots, type ScanOptions, type VaultFold } from "@estoc/vault/v3";
...
export async function openVault(driver: SqliteDriver, unlock: Unlock, options: VaultOptions = {}): Promise<OpenedVault> {
  ...
  const db = await openRuntime(driver, {
    anchor: async (wrapped) => {
      if (seedKey === undefined) seedKey = await unlockSeedKeystore({ version: 3, seedJwe: wrapped.seedJwe, keys: [] }, (unlock as { passphrase: string }).passphrase);
