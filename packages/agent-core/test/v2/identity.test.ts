...
import { MemoryBackend, NotAVault } from "@estoc/event-store";
import { createSeedKeystore } from "@estoc/keystore";
import { MemoryBackend as V1MemoryBackend } from "@estoc/vault";
import { KEY_ANCHOR, mediationKeyName } from "@estoc/vault/v2";

import { createVault as createV1Vault } from "../../src/index.js";
import { createVault, inspectVault, mintPeerDid, openVault } from "../../src/v2/index.js";
...
describe("v2 identity: did:peer:4 over the folder", () => {
  ...
  it("inspects without the seed: the folder and the sealed keystore; a v1 folder, or none, is not a vault", async () => {
    ...
    // a version-1 vault, laid down by the v1 agent, copied file by file
    const v1 = new V1MemoryBackend();
    const other = await freshKeystore();
    await createV1Vault(v1, { keystore: other.doc, seedKey: other.seedKey, label: "Alice" });
    const asV2 = new MemoryBackend();
    for (const [path, bytes] of v1.files) {
      await asV2.write(path, bytes);
    }
    await expect(inspectVault(asV2)).rejects.toThrow(NotAVault);
    await expect(openVault(asV2, other.seedKey)).rejects.toThrow(NotAVault);
  });
});
