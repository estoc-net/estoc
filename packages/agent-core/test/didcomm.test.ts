...
import { Message as UpstreamMessage } from "didcomm-node";
...
async function rotatedBob() {
  ...
  const [proof] = await new FromPrior({ iss: BOB, sub: successor.did }).pack(`${BOB}#auth`, side, secretsResolverFor(predecessor.secrets));
