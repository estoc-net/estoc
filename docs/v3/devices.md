...
(`vault-folder.md` §10), which the first version does by **pushing
deltas**: each device keeps, per sibling, the change token
(`event-store.md` §4.4) of its own store as of its last push, and on
open, on a timer, and after every append calls `changes(since)`, sends
what came, and keeps the new token. The token is local and
device-internal, as §4.4 requires; nothing of it crosses the wire.
...
- `body.events` — envelopes as `event-store.md` §2 defines them, in no
  ...
  with their blocks; the rest in the next message. The receiver
  `ingest`s (`event-store.md` §4.2): duplicates skipped, conflicts
...
Sync goes only to live siblings: a retired device is sent nothing and
what it sends is folded as its events are — suspect (`vault-events.md`
§5). A device that has been apart long enough to have missed pushes —
a token the sibling's store no longer places is not possible, since
events are never deleted, but a device may be new to a sibling that
already pushed to others — is caught up by a push from `since = null`,
everything, which is the snapshot by another road; the first version
prefers the zip. Anti-entropy proper — "what do you have that I do
not", by `eid` sets — is what replaces pushing if pushing proves
lossy (`event-store.md` §12); nothing above the seam changes for it.
...
- **Sync** (§5.3) is provisional throughout: push against a local token,
  blocks inline or by package, files not carried, zip to bootstrap.
  Anti-entropy by `eid` sets, a `want` for blocks, and a first sync
  that replaces the zip are the three likely changes, none of which
  touches an event type here.
